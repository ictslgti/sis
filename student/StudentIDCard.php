...
if (is_file($pref)) {
    ...
    foreach ($try as $p) { if (is_file($p)) { $principalSig = $__base . 'img/' . basename($p); break; } }
}
$ministryLogo = '';
$tryMin = [
...
foreach ($tryMin as $p) { if (is_file($p)) { $ministryLogo = $__base . 'img/' . basename($p); break; } }
...
<style>
:root{ --black:#111; --red:#e01e37; --gold:#ffce00; --card-w:85.6mm; --card-h:54mm; --fs-base:8px; --fs-head:18px; --fs-backhead:14px; --fs-name:12px; --fs-label:10px; --fs-val:10px; --qr-size:32mm; --photo-w:18mm; --photo-h:23mm; }
body{ background:#f4f5f7; }
.wrapper{ max-width:100%; padding:0 16px 16px; }
.grid{ display:grid; grid-template-columns:1fr; gap:16px; }
@media(min-width:992px){ .grid{ grid-template-columns:repeat(2, var(--card-w)); justify-content:center; } }
.card-iso{ width:var(--card-w); height:var(--card-h); border-radius:6mm; overflow:hidden; box-shadow:0 6px 20px rgba(0,0,0,.15); background:#fff; position:relative; border:1px solid #e9ecef; }
.card-iso, .card-iso *{ font-size:8px !important; }
.card-iso .inst-name,
.card-iso .heading,
.card-iso .name,
.card-iso .chip,
.card-iso .label,
.card-iso .val,
.card-iso .smalltxt{ margin-top:0 !important; line-height:1.1; }
.card-iso .rowline{ margin-top:0 !important; }
.card-iso .back-head{ margin-top:0 !important; }
.card-iso .fields{ line-height:1.1; }
.card-iso .rowline{ gap:2mm; }
.face{ position:relative; width:100%; height:100%; padding:10px 6.5mm 6.5mm 6.5mm; display:flex; flex-direction:column; justify-content:space-between; box-sizing:border-box; }
.topline{ height:3mm; width:34mm; background:linear-gradient(90deg, var(--black) 0 34%, var(--red) 34% 67%, var(--gold) 67% 100%); border-radius:2mm; }
.brand-line{ display:flex; align-items:center; justify-content:flex-end; margin-top:0; min-height:6.2mm; }
.brand-right{ display:flex; align-items:center; gap:2.2mm; margin-top:0; }
.brand-right img{ height:6.5mm; width:auto; object-fit:contain; }

.slgti-txt{ font-weight:700; font-size:7.6pt; white-space:nowrap; line-height:1; }
...
.inst-name{ text-align:center; font-weight:700; font-size:16px !important; color: var(--red); letter-spacing:.2px; margin-top:-6px; white-space:nowrap; }
...
.fields{ flex:1; font-size:9pt; min-width:0; line-height:2; text-align:left !important; }
.name{ font-size:12px !important; font-weight:700; color:#111; margin-top:1mm; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
...
.sig img{ max-height:11mm; width:auto; display:block; margin:0 auto; }
.sig .cap{ border-top:1px solid #222; margin-top:2mm; font-size:8pt; font-weight:600; }
/* Validity layout helpers */
.validity{ margin-top:3mm; }
.validity-line{ line-height:1.2; }
.qrbox{ width:var(--qr-size); height:var(--qr-size); border:1px dashed rgba(0,0,0,.18); border-radius:2mm; display:flex; align-items:center; justify-content:center; background:#fff; overflow:hidden; }
.qrbox #qr img, .qrbox #qr canvas{ width:100% !important; height:100% !important; display:block; }
.notice{ font-size:8pt; color:#495057; }
.badge-status{ position:absolute; top:7mm; right:7mm; font-size:8pt; }
.back-head{ font-weight:700; font-size:14px !important; margin-top:4mm; }
.smalltxt{ font-size:8px !important; }
.qrurl{ word-break: break-all; line-height:1.2; }
.print-actions{ text-align:center; margin:12px 0 4px; }
/* Back-side tweaks to ensure neat fit */
.card-iso.back .face{ padding:10px 7mm 7mm; }
.card-iso.back .rowline{ gap:5mm; }
.card-iso.back .qrcol{ flex:0 0 41.6667%; max-width:41.6667%; }
.card-iso.back .fields{ flex:0 0 58.3333%; max-width:58.3333%; }
.card-iso.back .sig{ min-width:auto; margin-top:auto; padding-top:3mm; display:flex; flex-direction:column; align-items:flex-end; }
.card-iso.back .sig img{ margin-left:auto; margin-right:0; }
.card-iso.back .fields{ font-size:8.5pt; display:flex; flex-direction:column; line-height:1.3; }
.card-iso.back .smalltxt{ font-size:8pt; }
.card-iso.back .qrbox{ width:var(--qr-size); height:var(--qr-size); }
.card-iso:not(.back) .face > div:first-child{ margin-top:0; }
.card-iso.back .face > div:first-child{ margin-top:0; }
.card-iso.back .sig{ min-width:auto; }
.card-iso.back .sig .cap{ width:36mm; margin-left:auto; margin-right:0; }
@media print{
  body{ background:#fff; }
  .print-actions{ display:none!important; }
  .ctrls{ display:none!important; }
  .wrapper{ padding:0; }
  .grid{ grid-template-columns:1fr 1fr; gap:0; justify-content:initial; }
  .card-iso, .card-iso *{ font-size:8px !important; }
  .card-iso{ box-shadow:none; margin:0; page-break-inside:avoid; }
}
</style>
<style>
/* Font-size overrides driven by variables (fixed for title/institute) */
.card-iso, .card-iso *{ font-size: var(--fs-base) !important; }
.inst-name{ font-size: 16px !important; }
.heading{ font-size: var(--fs-head) !important; }
.id-title{ font-size: 12px !important; }
.back-head{ font-size: var(--fs-backhead) !important; }
.name{ font-size: var(--fs-name) !important; }
.label{ font-size: var(--fs-label) !important; }
.val{ font-size: var(--fs-val) !important; }
.smalltxt{ font-size: var(--fs-base) !important; }
/* Controls UI */
.ctrls{ display:flex; flex-wrap:wrap; gap:8px; align-items:center; justify-content:center; margin-top:8px; }
.ctrls label{ margin:0 4px 0 0; font-weight:600; }
.ctrls input{ width:56px; padding:2px 4px; }
.ctrls .group{ display:flex; align-items:center; gap:6px; }
.ctrls .btn{ padding:4px 8px; }
</style>
</head>
<body>
<div class="wrapper">
  ...
  <div class="ctrls">
    <div class="group"><label for="fs-base">Base</label><input id="fs-base" type="number" min="6" max="20" step="1"></div>
    <div class="group"><label for="fs-head">Head</label><input id="fs-head" type="number" min="10" max="28" step="1"></div>
    <div class="group"><label for="fs-backhead">Back</label><input id="fs-backhead" type="number" min="10" max="28" step="1"></div>
        ...
        <div>
          ...
          <div class="brand-line">
            <div class="brand-right" style="margin-top:-30px;">
              <?php if ($ministryLogo !== ''): ?>
                <img src="<?php echo htmlspecialchars($ministryLogo); ?>" alt="Ministry">
              <?php endif; ?>
        ...
        <div>
          ...
          <div class="rowline" style="margin-top:4mm; align-items:flex-start;">
            ...
            <div class="fields" style="margin-top:-4mm;">
             ...
             <br> <div class="label" style="margin-top:3mm;">Validity</div> 
...
(function(){
  function renderQR(){
    ...
    if (url) {
      new QRCode(qre, { text: url, width: sz, height: sz, correctLevel: QRCode.CorrectLevel.M });
    }
  }
...
(function(){
  ...
  function initInputs(){
    document.getElementById('fs-base').value = parseInt(getVar('--fs-base','8px'));
    document.getElementById('fs-head').value = parseInt(getVar('--fs-head','18px'));
    document.getElementById('fs-backhead').value = parseInt(getVar('--fs-backhead','14px'));
  ...
  function bindAuto(){
    // Font-size inputs (px)
    ['fs-base','fs-head','fs-backhead','fs-name','fs-label','fs-val'].forEach(function(id){
    ...
    qrs.addEventListener('input', function(){ setVar('--qr-size', qrs.value + 'mm'); if (window.renderQR) window.renderQR(); });
    var pw = document.getElementById('photo-w');
    pw.addEventListener('input', function(){ setVar('--photo-w', pw.value + 'mm'); });
    var ph = document.getElementById('photo-h');
    ph.addEventListener('input', function(){ setVar('--photo-h', ph.value + 'mm'); });
  }
  function reset(){
    setVar('--fs-base','8px');
    setVar('--fs-head','18px');
  ...
  async function downloadZip(){
    ...
    zip.file('ID_'+id+'_front.png', b64_1, {base64:true});
    zip.file('ID_'+id+'_back.png', b64_2, {base64:true});
